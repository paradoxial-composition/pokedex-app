...
import { colors, PokemonType } from "../../../assets/typeColors";
...
const Card: React.FC<CardProps> = ({ pokemon, handleRedirection }) => {
  const typeTags = pokemon.types?.map((item, index) => (
    <Text
      key={index}
      style={[
        tw`mx-3 uppercase border rounded-3xl p-1 px-2`,
        { backgroundColor: colors[item?.type?.name as PokemonType] },
      ]}
    >
      {item?.type?.name}
    </Text>
  ...
  return (
    <View style={[tw`mb-5`]}>
      <TouchableOpacity
        onPress={() => handleRedirection("PokeDetails", pokemon)}
      >
        ...
        <View
          style={[
            tw`w-full p-2 m-auto flex justify-center border-2 rounded-b-3xl`,
            styles.infoCard,
          ]}
        >
          ...
          <View style={[tw`w-full flex-col`]}>
            <Text
              style={[tw`text-center my-2 uppercase text-3xl text-zinc-200`]}
            >
              {pokemon?.name}
            </Text>
          ...
          <View style={[tw`w-full flex flex-row justify-center my-2`]}>
            <Text style={[tw`mx-2 text-xl text-zinc-200`]}>{`Height: ${
              pokemon?.height ? pokemon?.height / 10 : "--"
            } m`}</Text>
            <Text style={[tw`mx-2 text-xl text-zinc-200`]}>{`Weight: ${
              pokemon?.weight ? pokemon?.weight / 100 : "--"
            } Kg`}</Text>
          </View>
        </View>
      </TouchableOpacity>
